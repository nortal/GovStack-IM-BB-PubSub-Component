...
class RejectSubscriptionService implements RejectSubscriptionUseCase {
    ...
    private void rejectSubscription(Subscription subscription) {
        var canApproveSubscription = VALID_STATUSES_FOR_APPROVAL.contains(subscription.getStatus());
        if (canApproveSubscription) {
            subscriptionStatusPort.setSubscriptionStatus(subscription.getId(), Status.REJECTED);
        } else {
            throw new ApiException("Subscription with status [" + subscription.getStatus() + "] can not be rejected");
        }
    }
}
