...
@ApiResponses({
    @ApiResponse(responseCode = "200", description = "Returns 200 when event type is created successfully"),
    @ApiResponse(
        responseCode = "404",
    ...
    @ApiResponse(
        responseCode = "409",
        description = "Returns 409 when event type already exists",
        content = {
            @Content(
                mediaType = "application/json",
                schema = @Schema(implementation = ApiErrorResponse.class),
                examples = {
                    @ExampleObject(
                        ...
                        value = "{ \"timestamp\": \"2023-08-11T15:53:57.5425895\", \"message\": \"" + EVENT_TYPE_ALREADY_EXISTS + "\" }")
                })
        }),
    @ApiResponse(
        responseCode = "500",
...
public @interface CreateEventTypeOperation {
}
