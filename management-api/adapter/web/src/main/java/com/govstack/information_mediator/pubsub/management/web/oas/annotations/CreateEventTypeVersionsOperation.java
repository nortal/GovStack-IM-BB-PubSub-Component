...
@ApiResponses({
    @ApiResponse(responseCode = "200", description = "Successfully creates new version for event type"),
    @ApiResponse(
        responseCode = "404",
        description = "Returns 404 when manager, room or event type is not found",
        content = {
            @Content(
    ...
    @ApiResponse(
        responseCode = "409",
        description = "Returns 409 when a version already exists for the event type",
        content = {
            @Content(
                mediaType = "application/json",
                schema = @Schema(implementation = ApiErrorResponse.class),
                examples = {
                    @ExampleObject(
                        name = "Manager not found",
                        value = "{ \"timestamp\": \"2023-08-11T15:53:57.5425895\", \"message\": \"" + EVENT_TYPE_VERSION_ALREADY_EXISTS + "\" }")
                })
        }),
    @ApiResponse(
        responseCode = "500",
...
public @interface CreateEventTypeVersionsOperation {
}
